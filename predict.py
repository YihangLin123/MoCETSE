...
def predict(model, fasta, batch_size, device, outdir, pos_labels, save_attn=False):
    ...
    result = pd.DataFrame({'name': names, 'system': systems, 'score': scores, 'NonSecreted.prob': probs_non_effector,
                           'T1SE.prob': probs_t1se,
                           'T2SE.prob': probs_t2se, 'T3SE.prob': probs_t3se, 'T4SE.prob': probs_t4se,
                           'T6SE.prob': probs_t6se, 'length': lengths})
