...
import esm
from MoCETSE.module import TransformerLayer, MoCE, MLPLayer


class EffectorTransformer(nn.Module):

    def __init__(self, emb_dim, repr_layer, num_layers, heads,
                 hid_dim=256, dropout_rate=0.4, num_classes=6, attn_dropout=0.05, return_embedding=False,
                 return_attn=False, max_length=1024):
        ...
        self.conv = MoCE(emb_dim, hid_dim, 1, 1, bias=False)
...
class ESM1bModel(nn.Module):
    ...
    def forward(self, strs, toks):
        toks = toks[:, :1022]
        batch = toks.shape[0]
        out = self.pretrained_model(toks, repr_layers=[self.repr_layer],
                                    return_contacts=False)  # (bs, seq_len, emb_dim)
        emb = torch.cat([out["representations"][33][i, 1: len(strs[i]) + 1].mean(0).unsqueeze(0) for i in range(batch)],
                        dim=0)
        if self.return_embedding:
            return emb
        else:
